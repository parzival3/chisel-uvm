...
class IfCon(val const: Constraint, val ifC: crv.Constraint, val thenC: crv.Constraint)(implicit model: Model)
  ...
  /** Companion class for creating if-then-else constraint
    ...
    */
  class IfElseCon(val ifCons: IfCon, val elseC: crv.Constraint) extends IfCon(const, ifC, thenC) {
  ...
  /** Create an if-then-else constraint
    ...
    * @return
    */
  def ElseC(elseC: crv.Constraint): IfElseCon = {
    const.disable()
...
object IfCon {
  ...
  /** If then constraint
    ...
    */
  def apply(ifC: crv.Constraint)(thenC: crv.Constraint)(implicit model: Model): IfCon = {
    val newConstraint =
